...
function [position_z_event,pfd_position_z, position_z] = ... % output arguments
    ...
    %% Finds the respective depths of the soil matrix corresponding to the depths of the three macropore sizes
        ...
        if theta(indx_big(1)-1) < ths(1) * 0.9
            big_sat = false;
        else
            big_sat = true;
        end
        ...
        if isnan(indx_mid(1)-1) || theta(indx_mid(1)-1) < ths(1) * 0.9
            mid_sat = false;
        else
            mid_sat = true;
        end
        ...
        if isnan(indx_sml(1)-1) || theta(indx_sml(1)-1) < ths(1) * 0.9
            sml_sat = false;
        else
            sml_sat = true;
        end
    ...
    % into soil matrix, no back diffusion from matrix into pfd
        if (pfd_theta(yy) == 1 && big_sat == false && mid_sat == false && sml_sat == false) || (yy == pfd_dim - 1 && pfd_theta(yy) > 0 && big_sat == false && mid_sat == false && sml_sat == false)
...
%         q_mix_psi = abs((2*ks(1)*((k(indx_big(1))+k(indx_big(1)-1))/2))/(ks(1)+((k(indx_big(1))+k(indx_big(1)-1))/2))*(((psi(indx_big(1))+psi(indx_big(1)-1))/2)/(0.00001))); %diffussive flow and mass calculated with psi_init
        q_mix_psi = abs((2*ks(1)*((k(1)+k(2))/2))/(ks(1)+((k(1)+k(2)/2)))*(((psi(1)+psi(2)/2)/(0.001)))); %diffussive flow and mass calculated with psi_init
        ...
        pfd_age_output = floor(sum(pfd_position_z(indx2(end)-pfd_particles_mix:indx2(end),2)) / pfd_particles_mix);
...
%                mix = [pfd_tmix_output * ones(length(pos_big),1); pfd_tmix_output * ones(length(pos_mid),1); pfd_tmix_output * ones(length(pos_sml),1)];
               ...
               mix = [randn(floor(length(pos)*t_mix_ratio),1) * t_mix_SD1 + t_mix_mean1;randn(ceil(length(pos)*(1-t_mix_ratio)),1) * t_mix_SD2 + t_mix_mean2];
               mix = mix(randperm(numel(mix)));
